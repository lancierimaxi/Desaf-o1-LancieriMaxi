...
class ProductManager {
	...
	async validateProduct(newProduct) {
		...
		Object.values(newProduct).forEach((value) => {
			if (value.trim() === "" || value === 0) {
				console.error("validateProduct: Error, por favor agregue un producto");
				return (validationResult = false);
			}
		});

		if (this.products.some((product) => product.code === newProduct.code)) {
			console.error("validateProduct: El código ya existe");
			return false;
		}
		...
		for (let key in productKeys) {
			if (Object.keys(newProduct)[key] !== productKeys[key]) {
				console.log(
					`validateProduct: El dato ${productKeys[key]} es requerido`
				);
				return false;
			}
		}
	...
	async validateProductId(productId) {
		...
		if (productId === 0 || productId > this.products.length) {
			console.log("validateProductId: Producto no encontrado");
			return false;
		}
		return validationResult;
	}
	...
	async getProductById(productId) {
		try {
			...
			if (validationResult) {
				...
				console.log("getProductById", filteredProduct);
